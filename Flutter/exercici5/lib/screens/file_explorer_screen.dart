...
class _FileExplorerScreenState extends State<FileExplorerScreen> {
  ...
  Future<void> _unzipRemote(String filename) async {
    ...
    try {
      ...
      await widget.client.run('unzip -o "$currentPath/$filename" -d "$currentPath/"');
    ...
    showDialog(
      ...
      builder: (c) => AlertDialog(
        ...
        content: Column(
          ...
          children: [
            _infoRow(Icons.calendar_today, "Modificat", DateFormat('dd/MM/yyyy HH:mm').format(date.toLocal())),
            _infoRow(Icons.lock_outline, "Permisos", (item.attr.mode?.value ?? 0 & 0xFFF).toRadixString(8)),
            _infoRow(Icons.data_usage, "Mida", "${((item.attr.size ?? 0) / 1024).toStringAsFixed(2)} KB"),
          ],
        ),
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        leading: directoryStack.isEmpty ? null : IconButton(
          icon: const Icon(Icons.arrow_back_ios_new, size: 20),
          onPressed: () { setState(() => currentPath = directoryStack.removeLast()); _load(); }
        ),
        title: Column(
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [
            Text(currentPath == '.' ? "Inici" : currentPath.split('/').last, 
                 style: const TextStyle(fontSize: 18, fontWeight: FontWeight.bold)),
            Text(widget.config.host, style: const TextStyle(fontSize: 11, color: Colors.blueGrey)),
          ],
        ),
        actions: [
          IconButton(icon: const Icon(Icons.bar_chart, color: Colors.blue), 
            onPressed: () => Navigator.push(context, MaterialPageRoute(builder: (c) => ServerStatsScreen(client: widget.client, currentPath: currentPath, items: items)))),
  ...
  Widget _buildTopActions() => Padding(
    ...
    child: Row(
      children: [
        Expanded(child: _actionBtn("Pujar Fitxer", Icons.upload_file, Colors.blue, _upload)),
        const SizedBox(width: 12),
        Expanded(child: _actionBtn("Nou Item", Icons.create_new_folder, Colors.blueGrey, _showCreateDialog)),
      ],
    ),
  );

  Widget _actionBtn(String t, IconData icon, Color col, VoidCallback f) => ElevatedButton.icon(
    onPressed: f,
    ...
    style: ElevatedButton.styleFrom(
      ...
      shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(12), side: BorderSide(color: col.withOpacity(0.2))),
    ),
  );
    ...
    return Container(
      ...
      child: ListTile(
        ...
        leading: Container(
          ...
          decoration: BoxDecoration(color: (isDir ? Colors.amber : Colors.blueGrey).withOpacity(0.1), borderRadius: BorderRadius.circular(10)),
          child: Icon(isDir ? Icons.folder : Icons.insert_drive_file, color: isDir ? Colors.amber.shade700 : Colors.blueGrey.shade700),
        ),
        ...
        trailing: Row(
          ...
          children: [
            ...
            if (isZip)
              IconButton(
                icon: const Icon(Icons.unarchive, color: Colors.orange, size: 20),
                tooltip: "Descomprimir al servidor",
                onPressed: () => _unzipRemote(item.filename),
              ),
            IconButton(icon: const Icon(Icons.download_rounded, size: 20, color: Colors.blueGrey), onPressed: () => _download(item, isDir)),
            PopupMenuButton<String>(
              icon: const Icon(Icons.more_vert, color: Colors.blueGrey),
              onSelected: (v) async {
                if (v == 'info') _showFileInfo(item);
                if (v == 'del') { 
                  if (isDir) await sftp.rmdir("$currentPath/${item.filename}");
                  else await sftp.remove("$currentPath/${item.filename}");
                  _load(); 
                }
              },
              itemBuilder: (c) => [
                const PopupMenuItem(value: 'info', child: Row(children: [Icon(Icons.info_outline, size: 18), SizedBox(width: 8), Text("Info")])),
                const PopupMenuItem(value: 'del', child: Row(children: [Icon(Icons.delete_outline, size: 18, color: Colors.red), SizedBox(width: 8), Text("Eliminar", style: TextStyle(color: Colors.red))])),
              ],
            ),
          ],
        ),
        onTap: isDir ? () { directoryStack.add(currentPath); setState(() => currentPath += "/${item.filename}"); _load(); } : null,
      ),
    );
  }
  ...
  Widget _serverBar() => Container(
    ...
    child: SafeArea(
      child: Row(
        children: [
          Icon(Icons.circle, color: isServerRunning ? Colors.green : Colors.red, size: 12),
          const SizedBox(width: 12),
          Expanded(child: Text("Port $serverPort ($serverType)", style: const TextStyle(fontWeight: FontWeight.w800, fontSize: 13))),
          IconButton(
            icon: Icon(isServerRunning ? Icons.restart_alt : Icons.play_arrow_rounded, color: Colors.blue),
            onPressed: () => _cmdServer(isServerRunning ? 'RESTART' : 'START'),
          ),
          if (isServerRunning) IconButton(
  ...
  void _showMsg(String m, {bool isError = true}) => ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text(m), backgroundColor: isError ? Colors.redAccent : Colors.green, behavior: SnackBarBehavior.floating));
    ...
    showDialog(context: context, builder: (c) => StatefulBuilder(builder: (c, setS) => AlertDialog(
      ...
      content: Column(mainAxisSize: MainAxisSize.min, children: [
        DropdownButtonFormField<String>(
          ...
          value: type, 
          items: ['Fitxer', 'Directori'].map((e) => DropdownMenuItem(value: e, child: Text(e))).toList(), 
          onChanged: (v) => setS(() => type = v!)
        ),
        const SizedBox(height: 12),
        TextField(controller: ctrl, decoration: const InputDecoration(hintText: "Nom", border: OutlineInputBorder())),
      ]),
      actions: [
