...
class _ServerStatsScreenState extends State<ServerStatsScreen> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      backgroundColor: const Color(0xFFF5F7FA), // Gris molt clar de fons professional
      appBar: AppBar(
        title: Text(widget.currentPath.split('/').last, 
        ...
        child: Column(
          children: [
            ...
            _buildSectionCard(
              ...
              child: Column(
                children: [
                  ...
                  const SizedBox(height: 10),
                  const Text("Toca els segments per veure detalls", 
            ...
            const CustomIndentList(
              ...
              subItems: ["NodeJS Engine v18.x", "Express Framework", "PM2 Process Manager", "Body-parser Middleware"],
            ),
  ...
  Widget _buildSectionCard({required String title, required Widget child}) {
    return Container(
      ...
      child: Column(
        ...
        children: [
          Text(title, style: TextStyle(fontSize: 12, fontWeight: FontWeight.w900, color: Colors.blueGrey.shade300, letterSpacing: 1.2)),
  ...
  Widget _buildStatusHeader() {
    return Row(
      children: [
        ...
        Expanded(
          child: Text(serverStatus.toUpperCase(), 
            style: TextStyle(color: isServerActive ? Colors.green.shade700 : Colors.red.shade700, 
  ...
  Widget _buildEditableConfig() {
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        const Text("Identificador de xarxa", style: TextStyle(fontSize: 14, color: Colors.black54)),
        TextField(
      ...
      children: [
        const Expanded(
          child: Column(
            crossAxisAlignment: CrossAxisAlignment.start,
            children: [
              Text("HTTP Redirecció (Port 80)", style: TextStyle(fontWeight: FontWeight.bold)),
              Text("Apunta el trànsit extern al port 3000", style: TextStyle(fontSize: 12, color: Colors.grey)),
            ],
          ),
        ),
        Switch.adaptive(
      ...
      child: Column(
        ...
        children: [
          Padding(
            ...
            child: Text(title, style: TextStyle(fontSize: 12, fontWeight: FontWeight.w900, color: Colors.blueGrey.shade300, letterSpacing: 1.2)),
          ),
          const SizedBox(height: 10),
          ...subItems.map((item) => Container(
...
class RealBaobabPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    final List<Color> palette = [
      const Color(0xFF6C5CE7), const Color(0xFF00CEC9), const Color(0xFFFAB1A0),
      const Color(0xFF0984E3), const Color(0xFFFD79A8), const Color(0xFF00B894),
    ];
    ...
    for (var i = 0; i < items.length; i++) {
      ...
      if (hoverPosition != null) {
        ...
        if (dist > radius - 30 && dist < radius + 30) {
          double touchAngle = atan2(hoverPosition!.dy - center.dy, hoverPosition!.dx - center.dx);
      ...
      paint.color = palette[i % palette.length].withOpacity(isHovered ? 1.0 : 0.7);
    ...
    _drawCenterText(canvas, center, hoveredName ?? "${(totalSize / 1024).toStringAsFixed(1)} KB");
  }
  ...
  void _drawCenterText(Canvas canvas, Offset center, String text) {
    final tp = TextPainter(
      text: TextSpan(
        text: text, 
        style: const TextStyle(color: Color(0xFF2D3436), fontWeight: FontWeight.w900, fontSize: 15)
      ),
